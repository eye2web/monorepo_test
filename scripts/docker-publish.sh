...
check_artifactory_image () {
    echo "Checking if container with name ($COMPONENT_NAME) and version ($COMPONENT_VERSION) already exists"
    ...
    echo "Container with name ($COMPONENT_NAME) version ($COMPONENT_VERSION) exists=($CONTAINS_VERSION)"
};

build_image () {
    # Build image
    echo "Building docker image with name: ($DOCKER_REPOSITORY$COMPONENT_NAME:$COMPONENT_VERSION)"
    docker build --build-arg version=${COMPONENT_VERSION} --build-arg name=${COMPONENT_NAME} -t $DOCKER_REGISTRY$COMPONENT_NAME:$COMPONENT_VERSION $DOCKER_FILE
};
